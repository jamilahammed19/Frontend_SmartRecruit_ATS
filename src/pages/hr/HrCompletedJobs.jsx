...
import { getJobs, updateJobStatus } from '../../services/jobService'; // Imported updateJobStatus

export default function HrCompletedJobs() {
    const [completedJobs, setCompletedJobs] = useState([]);
    const [loading, setLoading] = useState(true);
    const [updatingId, setUpdatingId] = useState(null); // Track which job is being updated
    ...
    useEffect(() => {
        const fetchJobs = async () => {
            try {
                ...
                // Filter specifically for completed jobs
    ...
    // --- NEW: Function to Uncomplete / Reopen a job ---
    const handleReopen = async (jobId) => {
        if (window.confirm("Are you sure you want to reopen this job? It will be moved back to the Pending Processes dashboard.")) {
            setUpdatingId(jobId);
            try {
                // Change status back to processing
                await updateJobStatus(jobId, { status: 'processing' });
                
                // Instantly remove it from the completed list on the screen
    ...
    if (loading) return <div className="p-10 text-center text-slate-500">Loading Completed Jobs...</div>;

    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 font-sans">
            <div className="mb-8">
                <h1 className="text-3xl font-bold text-slate-900">Completed Jobs</h1>
                <p className="text-slate-500 mt-2">History of successfully closed recruitment pipelines.</p>
            </div>
                    ...
                    <tbody className="text-sm divide-y divide-slate-100">
                        {completedJobs.length === 0 ? (
                            ...
                            completedJobs.map((job) => (
                                <tr key={job.id} className="hover:bg-slate-50 transition-colors opacity-90">
