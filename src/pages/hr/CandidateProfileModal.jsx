    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[95vh] flex flex-col relative overflow-hidden">
        ...
        <div className="p-6 border-b border-slate-200 bg-slate-50 flex justify-between items-start flex-shrink-0">
          <div className="flex items-center space-x-5">
            ...
            <div>
              <h2 className="text-2xl font-black text-slate-900">
                {fp.full_name || candidate.candidate_name}
              </h2>
              <div className="flex items-center space-x-3 text-sm text-slate-600 mt-1 font-medium">
                {(fp.verified_email || candidate.candidate_email) && (
                  <span>
                    ✉️ {fp.verified_email || candidate.candidate_email}
                  </span>
                )}
          ...
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-slate-700 bg-white rounded-full p-2 border hover:bg-slate-100 transition-colors"
          >
            ✕
          </button>
        </div>
        ...
        <div className="flex-grow overflow-hidden flex flex-col md:flex-row">
          <div className="w-full md:w-1/3 bg-slate-50 border-r border-slate-200 p-6 overflow-y-auto space-y-8">
            <div
              className={`p-6 rounded-2xl border shadow-sm text-center ${candidate.ai_match_score >= 800 ? "bg-emerald-50 border-emerald-200" : candidate.ai_match_score === null ? "bg-white border-slate-200" : "bg-amber-50 border-amber-200"}`}
            >
              <div className="text-xs uppercase font-black tracking-wider mb-2 opacity-75">
                AI Match Score
              </div>
              ...
              <div className="text-sm font-bold mt-1 opacity-75">
                Out of 1000
              </div>
            </div>
            ...
            <div>
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm divide-y divide-slate-100">
                ...
                {fp.marital_status && (
                  <div className="flex justify-between p-3 text-sm">
                    <span className="text-slate-500 font-medium">Marital</span>
                    <span className="font-bold text-slate-900 capitalize">
                      {fp.marital_status}
                    </span>
                  </div>
          ...
          <div className="w-full md:w-2/3 p-6 md:p-8 overflow-y-auto space-y-10 bg-white">
            ...
            {fp.skills?.length > 0 && (
              <section>
                <h3 className="text-lg font-black text-slate-900 mb-4 border-b pb-2 flex items-center gap-2">
                  ⚡ Technical Skills
                </h3>
                <div className="flex flex-wrap gap-2">
                  {fp.skills.map((skill, i) => (
                    <span
                      key={i}
                      className="px-4 py-2 bg-slate-50 text-slate-800 text-sm font-bold rounded-lg border border-slate-200 shadow-sm"
                    >
                      {skill.skill_name}{" "}
              ...
              <section>
                ...
                <div className="space-y-3">
                  {fp.trainings.map((trn, i) => (
                    <div
                      key={i}
                      className="flex justify-between items-center p-3 rounded-lg border border-slate-100 hover:bg-slate-50 transition-colors"
                    >
                      <div>
                        ...
                        <div className="text-xs text-slate-500">
                          {trn.institute} {trn.location && `• ${trn.location}`}
                        </div>
                      </div>
                      ...
                      <h4 className="font-bold text-slate-900 flex justify-between items-start">
                        ...
                        {proj.link && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="text-blue-600 text-xs hover:underline ml-4 break-all"
                          >
                            🔗 View Link
                          </a>
                        )}
                ...
                <ul className="space-y-3">
                  {fp.extracurricular_activities.map((ext, i) => (
                    <li key={i} className="text-sm">
                      ...
                      {ext.description && (
                        <p className="text-slate-600 mt-1">{ext.description}</p>
                      )}
                    </li>
                  ))}
                </ul>
        ...
        <div className="p-5 border-t border-slate-200 bg-white flex justify-between items-center flex-shrink-0 z-50">
          <span
            className={`px-4 py-2 rounded-lg text-sm font-black uppercase tracking-wider ${
