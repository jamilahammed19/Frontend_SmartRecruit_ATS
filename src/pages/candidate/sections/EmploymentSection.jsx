      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        ...
        <div className="space-y-4">
          {!items || items.length === 0 ? (
            <p className="text-sm text-slate-500">No employment records added yet.</p>
          ) : (
            items.map((emp) => (
              <div key={emp.id} className="p-5 border border-slate-100 bg-slate-50 rounded-xl flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                ...
                <div className="flex-grow">
                  ...
                  <div className="text-sm text-slate-700 font-medium mt-1">
                      ...
                      {emp.organization_location && ` • ${emp.organization_location}`}
                  </div>
                  
                  {/* Dynamic Details Row */}
                  <div className="text-sm text-slate-500 mt-2 flex flex-wrap gap-x-5 gap-y-1">
                      {emp.organization_business && <span><strong>Industry:</strong> {emp.organization_business}</span>}
                      <span><strong>Duration:</strong> {emp.start_date || "Unknown"} to {emp.is_current ? 'Present' : (emp.end_date || "Unknown")}</span>
                  </div>
                ...
                <div className="flex space-x-3 sm:ml-4 flex-shrink-0">
                  <button onClick={() => onEdit(emp)} className="text-sm font-bold text-blue-600 hover:text-blue-800">
                    Edit
                  </button>
                  <button onClick={() => onDelete("employment", emp.id)} className="text-sm font-bold text-red-600 hover:text-red-800">
                    Delete
                  </button>
                </div>
