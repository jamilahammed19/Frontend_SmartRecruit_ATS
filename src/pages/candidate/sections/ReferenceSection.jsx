export default function ReferenceSection({ items, onAdd, onEdit, onDelete }) {
    return (
              ...
              <div key={item.id} className="p-5 border border-slate-100 bg-slate-50 rounded-xl flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                ...
                <div className="flex-grow">
                  ...
                  <div className="text-sm text-slate-500 mt-2 flex flex-wrap gap-x-5 gap-y-1">
                    {item.relationship && <span><strong>Relationship:</strong> {item.relationship}</span>}
                    {item.mobile_number && <span><strong>Mobile:</strong> {item.mobile_number}</span>}
                    {item.phone_office && <span><strong>Office:</strong> {item.phone_office}</span>}
                    {item.email && <span><strong>Email:</strong> {item.email}</span>}
                  </div>

                  {item.address && (
                      <p className="text-sm text-slate-600 mt-2"><strong>Address:</strong> {item.address}</p>
                  )}
                </div>
                ...
                <div className="flex space-x-3 sm:ml-4 flex-shrink-0">
                  ...
                  <button onClick={() => onDelete("reference", item.id)} className="text-sm font-bold text-red-600 hover:text-red-800">
                    Delete
                  </button>
                </div>
