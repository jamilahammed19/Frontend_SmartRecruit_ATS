...
export default function CandidateJobList() {
    ...
    const handleSubmitApplication = async (e) => {
        ...
        try {
            ...
            setApplyError(err.response?.data?.detail || err.response?.data?.non_field_errors || "Failed to submit application.");
        } finally {
            setApplying(false);
        }
    };
    ...
    return (
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 font-sans">
            <div className="mb-8">
                ...
                <p className="mt-2 text-slate-500">Browse open positions, view details, and apply.</p>
            </div>

            {loading ? <div className="text-center py-12 text-slate-500">Loading open positions...</div> : error ? (
                ...
                <div className="space-y-4">
                    {jobs.map((job) => {
                        ...
                        return (
                            <div key={job.id} onClick={() => handleOpenDetails(job)} className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 hover:border-blue-400 hover:shadow-md cursor-pointer flex justify-between items-center group transition-all">
                                <div>
                                    <h2 className="text-lg font-bold text-slate-900 group-hover:text-blue-600">{job.title}</h2>
                                    <div className="flex items-center space-x-2 text-sm text-slate-500 mt-1">
                                        <span>{job.department}</span>
                                        <span>•</span>
                                        <span>{job.location}</span>
                                        <span>•</span>
                                        <span className="text-amber-600 font-medium">Closes: {job.deadline}</span>
                                    </div>
                                </div>
                                <div>
                                    {hasApplied ? (
                                        <span className="px-3 py-1 text-xs font-bold bg-emerald-100 text-emerald-700 rounded-full border border-emerald-200">✓ Applied</span>
                                    ) : (
                                        <span className="px-4 py-2 text-sm font-medium bg-slate-50 text-slate-600 border rounded-lg group-hover:bg-blue-50 group-hover:text-blue-600 transition-colors">View Details</span>
                                    )}
                                </div>
                            </div>
                        );
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col relative overflow-hidden">
                        ...
                        <div className="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-start">
                            <div>
                                ...
                                <p className="text-sm text-slate-500 mt-1">{selectedJob.department} • {selectedJob.location}</p>
                            </div>
                            ...
                            {modalMode === 'view' ? (
                                <div className="space-y-6">
                                    <div>
                                        ...
                                        <p className="text-slate-600 whitespace-pre-line text-sm leading-relaxed">{selectedJob.description}</p>
                                    </div>
                                    {selectedJob.requirements && (
                                        <div>
                                            <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-2">Requirements</h3>
                                            <p className="text-slate-600 whitespace-pre-line text-sm leading-relaxed">{selectedJob.requirements}</p>
                                        </div>
                                    )}
                                </div>
                            ) : (
                                <div className="space-y-4">
                                    <div className="bg-blue-50 border border-blue-100 p-4 rounded-lg">
                                        <p className="text-sm text-blue-800">Your SmartRecruit CV profile will be automatically attached to this application.</p>
                                    </div>
                                    {applyError && <div className="p-3 bg-red-50 text-red-700 text-sm rounded-lg border border-red-100">{applyError}</div>}
                                    <div>
