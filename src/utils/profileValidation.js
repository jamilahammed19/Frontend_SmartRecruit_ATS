export const checkProfileCompletion = (profile) => {
    ...
    const hasPersonalInfo = !!(
        pInfo.full_name && pInfo.father_name && pInfo.mother_name &&
        pInfo.date_of_birth && pInfo.gender && pInfo.religion &&
        pInfo.marital_status && pInfo.nationality && pInfo.nid &&
        pInfo.phone_number && pInfo.blood_group
    );
    
    // 3. Helper for Address completeness
    const isAddressComplete = (addr) => {
        if (!addr) return false;
        return !!(addr.country && addr.division && addr.district && addr.thana_upzila && addr.post_office && addr.post_code && addr.house_road_village);
    };
    ...
    const hasSSC = educations.some(edu => edu.degree_type?.toLowerCase() === 'ssc');
    const hasHSC = educations.some(edu => edu.degree_type?.toLowerCase() === 'hsc');
